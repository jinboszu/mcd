...
final public class DecompPoly extends Anim2D {
    ...
    public void init(boolean bugfix) {
        initReflex();
        subD = new SubDecomp(reflexFlag);
        initVisibility(bugfix);
        initSubproblems();
    }
    ...
    public void initVisibility(boolean bugfix) { // initReflex() first
        if (animating())
    ...
    private void setAfter(int i) { // i reflex
        massert(reflex(i), "non reflex i in setAfter(" + i + ")");
        ...
        if (reflex(i)) {
            ...
            if (j - i > 1) {
                if (pair.aB() != pair.bB()) {
                    ...
                    massert(!pd.emptyB(), "emptied pd " + i + "," + j + "," + k + " " + pair.toString());
                }
                recoverSolution(i, j);
            }
        } else {
            ...
            if (k - j > 1) {
                if (pair.aF() != pair.bF()) {
                    ...
                    massert(!pd.empty(), "emptied pd " + i + "," + j + "," + k + " " + pair.toString());
                }
                recoverSolution(j, k);
            }
        }
    ...
    public void typeA(int i, int j, int k) { /* i reflex; use jk */
        //    System.out.print("\nA "+i+","+j+","+k+":");
        //    massert(reflex(i), "non reflex i in typeA("+i+","+j+","+k+")");
        //    massert(k-i > 1, "too small in typeA("+i+","+j+","+k+")");
    ...
    */
    ...
    public void massert(boolean flag, String s) {
