...
public class VisPoly extends Anim2D {
     ...
     */
    public void build(int orgIndex, boolean bugfix) {
        if (bugfix) {
            build_bugfix(orgIndex);
        } else {
            build(orgIndex);
        }
    }
    ...
    public void build_bugfix(int orgIndex) {
        Homog edgej;        // during the loops, this is the line p(j-1)->p(j)
        org = sp.p(orgIndex);
        vp = -1;
        int j = orgIndex;
        ...
        do {            // loop always pushes pj and increments j.
            ...
            if (j >= sp.number() + orgIndex) return; // we are done.
            edgej = p(j - 1).meet(p(j));
            if (!edgej.right(org)) continue; // easiest case: add edge to VP.
            // else pj backtracks, we must determine where
            if (edgej.right(p(j - 2))) {// pj is above last VP edge
                j = exitRBay_bugfix(j, top(), Homog.INFINITY);
                push(j++, RLID);
                continue;  // exits bay; push next two
            }
            ...
            do {        // pj hides some of VP; break loop when can push pj.
                //System.out.print("do j:"+j+" lid:"+LLidIdx+" "+RLidIdx+toString());
                if (!Point.right(org, top(), p(j))) {// saved lid ok so far...
                    if (Point.right(p(j), p(j + 1), org)) j++; // continue to hide
                    else if (!edgej.right(p(j + 1))) { // or turns up into bay
                        j = exitLBay_bugfix(j, p(j), p(LLidIdx).meet(p(LLidIdx - 1))) + 1;
                    } else {    // or turns down; put saved lid back & add new VP edge
                        restoreLid();
                        push(j++, LWALL);
                        break;
                    ...
                    if (edgej.right(top())) { // entered RBay, must get out
                        massert((RLidIdx != NOTSAVED),
                                "no RLid saved " + LLidIdx + RLidIdx + toString());
                        j = exitRBay_bugfix(j, top(), edgej.neg()); // exits bay;
                        push(j++, RLID);
                        break;
                    } // found new visible lid to add to VP.
                    else saveLid(); // save new lid from VP; continue to hide VP.
    ...
    public static void massert(boolean flag, String s) {
    ...
    int exitRBay_bugfix(int j, Point bot, Homog lid) {
        int wn = 0;        // winding number
        Homog mouth = org.meet(bot);
        boolean lastRight, currRight = true;
        while (++j < 3 * sp.number()) {
            lastRight = currRight;
    ...
    int exitLBay_bugfix(int j, Point bot, Homog lid) {
        int wn = 0;        // winding number
        Homog mouth = org.meet(bot);
        boolean lastRight, currRight = false; // called with !right(org,bot,pj)
        while (++j < 3 * sp.number()) {
