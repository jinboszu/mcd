...
public abstract class PointIOApplet extends Applet {
    ...
    private Button example1_button = new Button("Example 1");
    private Button example2_button = new Button("Example 2");
    private Button solve_button = new Button("Solve");

    final public boolean animating() {
        return animating;
    }
    ...
    void addbuttons(Panel p) {
        ...
        p.add(random_button);
        p.add(update_button);
        p.add(example1_button);
        p.add(example2_button);
        p.add(solve_button);
    }
    ...
    boolean handlebuttons(Object target) {
        ...
        } else if (target == example1_button) {
            pts.removeAll();
            result = null;
            int[][] points = {{0, 2}, {0, 0}, {3, 0}, {3, 1}, {2, 1}, {1, 1}};
            for (int[] point : points) {
                int x = 150 + point[0] * 100;
                int y = 250 - point[1] * 100;
                pts.add(x, y);
            }
        } else if (target == example2_button) {
            pts.removeAll();
            result = null;
            int[][] points = {{1, 1}, {0, 1}, {1, 0}, {2, 1}, {3, 1}, {2, 2}};
            for (int[] point : points) {
                int x = 150 + point[0] * 100;
                int y = 250 - point[1] * 100;
                pts.add(x, y);
            }
        } else if (target == solve_button) {
            if (!pts.isEmpty()) {
                CompGeom.Point.reset();
                bugfix = true;
                result = compute(pts);
