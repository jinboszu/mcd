...
public class PointList extends Anim2D {
    ...
    protected void dr() {
        if (animating()) draw(animGraphics, true);
    }

    public void add(int x, int y) {
        p[counter] = new Point(x, y);
        if ((counter <= 0) || (!p[counter - 1].equals(p[counter]))) counter++;
        dr();
    }
    ...
    final public int leftmost() {
        ...
        if (animating()) p(min).drawCirc(animGraphics, 5);
        dr();
        return min;
    }

    final public int extreme(Point dir) {
